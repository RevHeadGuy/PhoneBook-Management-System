...
cout<<"ENTER G-MAIL :";
cin>>gmail;
temp=new dnode(name,number,gmail);// Dynamic allocation of memory
if(head==NULL)
{
head=temp;
}
...
{
temp=strcmp(i->name,j->name);// compare two strings
if(temp>0)
{
strcpy(n,i->name); // copy two strings
...
void dlist::deletecontact(char s[20])if(ptr==head)
{
head=head->next;
head->prev=NULL;
delete(ptr);
...
DETAILS ON EXISTING CONTACT\n4) DELETE CONTACT\n5) DELETE SAME NAME IN
